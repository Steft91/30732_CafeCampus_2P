...
| **Exception Filters / excepciones HTTP** | Framework (excepciones) + equipo (uso consistente) | `pedidos.service.ts` (`HttpException`, `NotFoundException`), `benchmark.service.ts` (`ServiceUnavailableException`) | Un error no derriba el proceso: se traduce a respuesta HTTP (422, 404, 503). |
...
| **DIP (Inversión de Dependencias)** | servicios reciben dependencias por constructor (`PrismaService`) | Dependen de abstracciones inyectadas, no de instancias creadas a mano. |
...
## Lectura rápida para la defensa

- Lo que **Nest da gratis**: IoC/DI, módulos, `ValidationPipe`, jerarquía de excepciones,
  transportes TCP/Redis de `@nestjs/microservices`.
- Lo que **diseñó el equipo**: el rol de API Gateway, los Proxy HTTP, el **contraste deliberado
  sync (TCP encadenado) vs async (Redis pub/sub)** para medir latencia y acoplamiento, el
  aislamiento por schema, y el manejo de errores consistente que traduce fallos a 422/404/503.
