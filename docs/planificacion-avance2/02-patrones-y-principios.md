...
| **DIP (Inversión de Dependencias)** | `pedidos.service.ts` depende de la **interfaz** `ProductosGrpcService` (tipo TS derivado del `.proto`), no de una implementación concreta | El cliente habla contra el contrato; el servidor puede cambiar internamente mientras respete el `.proto`. |
| **OCP (Abierto/Cerrado)** | `ms-inventario` **agrega** un tercer transporte (RMQ) junto a TCP y Redis en `main.ts` | Se añade un transporte nuevo sin modificar los caminos TCP/Redis existentes del Avance 1. |
...
| **Aislamiento de fallos (resiliencia)** | `.pipe(timeout(...))` en toda llamada saliente (gRPC 3000 ms, RMQ 1500 ms) | Ninguna dependencia lenta bloquea indefinidamente al servicio llamante. |
